...
class EncoderInterface(ModuleInterface):
    ...
    def parse_logged_data(
        log_data: dict[Any, list[dict[str, Any]]], cm_per_pulse: np.float64
        ...
        Args:
            log_data: A dictionary that stores the data extracted from the compressed log archive generated by the
...
class TTLInterface(ModuleInterface):
    ...
    @staticmethod
    def parse_logged_data(log_data: dict[Any, list[dict[str, Any]]]) -> NDArray[np.uint64]:
        """Extracts and prepares the data acquired by the module during runtime for further analysis.
...
class BreakInterface(ModuleInterface):
    ...
    @staticmethod
    def parse_logged_data(
        log_data: dict[Any, list[dict[str, Any]]],
...
class ValveInterface(ModuleInterface):
    ...
    @staticmethod
    def parse_logged_data(
        log_data: dict[Any, list[dict[str, Any]]], scale_coefficient: np.float64, nonlinearity_exponent: np.float64
    ) -> tuple[NDArray[np.uint64], NDArray[np.float64]]:
        """Extracts and prepares the data acquired by the module during runtime for further analysis.
...
class LickInterface(ModuleInterface):
    ...
    @staticmethod
    def parse_logged_data(
        log_data: dict[Any, list[dict[str, Any]]], lick_threshold: int
...
class TorqueInterface(ModuleInterface):
    ...
    @staticmethod
    def parse_logged_data(
        log_data: dict[Any, list[dict[str, Any]]], torque_per_adc_unit: np.float64
    ) -> tuple[NDArray[np.uint64], NDArray[np.float64]]:
        """Extracts and prepares the data acquired by the module during runtime for further analysis.
...
class ScreenInterface(ModuleInterface):
    ...
    @staticmethod
    def parse_logged_data(
        log_data: dict[Any, list[dict[str, Any]]], initially_on: bool
    ) -> tuple[NDArray[np.uint64], NDArray[np.uint8]]:
        """Extracts and prepares the data acquired by the module during runtime for further analysis.
        ...
        if not on_data:
            ...
            return np.array([off_data[0]["timestamp"]], dtype=np.uint64), np.array([initially_on], dtype=np.uint8)
