...
class _MesoscopeVRSystem:
    ...
    def _setup_unity(self) -> None:
        ...
        outcome = ""
        while outcome != "abort":
            # Blocks for at most 10 minutes (600 seconds, 600000 milliseconds) at a time.
            delay_timer.reset()
        ...
        # be working, does not use timeout or abort logic.
        while True:
            if self._unity.has_data:
                topic, _ = self._unity.get_data()
                if topic == self._unity_startup_topic:
    ...
    def _deliver_reward(self, reward_size: float = 5.0) -> None:
        ...
        # Configures the visualizer to display the valve activation evnet during the next update cycle.
    ...
    def _data_cycle(self) -> None:
        ...
        # Since position updates are used exclusively by Unity game engine, skips position update tracking if Unity
        # support is disabled for this runtime.
        if self._unity is not None:
        ...
        if lick_count > self._lick_count:
            ...
            # If this runtime uses Unity, also notifies Unity about detected lick event.
            if self._unity is not None:
        ...
        if self._speed_timer.elapsed >= self._speed_calculation_window:
            ...
            delta_distance = travelled_distance - self._distance

            # Converts from cm / ms to cm / s and casts to the typy expected by the visualizer class.
            running_speed = np.float64((delta_distance / 100) * 1000)

            # Caches the new traveled distance and the running speed value to class attributes.
            self._distance = travelled_distance
    ...
    def _mesoscope_cycle(self) -> None:
        ...
        the emergency pause state, similar to how Unity termination messages are handled by _unity_cycle() method.
        """
