...
class _MesoscopeVRSystem:
    ...
    Attributes:
        ...
        _runtime_state: Stores the current runtime state-code.
        ...
        _lick_count: Stores the lick count used during the last communication cycle with Unity game engine.
        _cue_sequence: Stores the Virtual Reality wall cue sequence used by the currently active Unity task environment.
        _unconsumed_reward_count: Stores the current number of rewards delivered to the animal without the animal
            consuming them.
        ...
        _reward_volume: Stores the volume of water dispensed to the animal as part of each reward delivery.
        ...
        _ui: Optionally stores the RuntimeControlUI instance used during runtime to allow the user to interface with the
            Mesoscope-VR system via QT6 GUI. If the managed runtime does not use a GUI, this attribute is set to None.
    ...
    def __init__(
        ...
        # Defines optional assets used by some, but not all runtimes. The se assets are initialized to None by default
        # and are overwritten by the start_runtime() method.
    ...
    def start_runtime(self) -> None:
        """Starts the Mesoscope-VR system runtime (session data acquisition) by initializing additional assets and
        starting all data acquisition processes.

        This secondary initialization method is designed to be called just before entering the main runtime cycle of
        the logic function that calls this method. Depending on runtime configuration (session type), it initializes
        the necessary external assets (Unity, Mesoscope), internal assets (GUI, Visualizer) and guides the user through
        final runtime preparation steps.
        """

        # Makes it impossible to start the runtime unless class assets have been initialized (started).
        if not self._started:
            message = (
                "Unable to start the Mesoscope-VR system runtime as primary runtime assets have not been initialized. "
                "Call the class start() method before calling this method."
            )
            console.error(message=message, error=RuntimeError)
        ...
        # Reads important Unity task states from the GUI and caches them to class attributes.
    ...
    def runtime_cycle(self) -> None:
        """Sequentially calls all Mesoscope-VR runtime cyclic methods.

        This base cycle method should be called by the runtime logic function as part of its main runtime loop. Calling
        this method synchronizes various assets used by the class instance, such as the GUI, Unity game engine, and
    ...
    def _unity_cycle(self) -> None:
        ...
        This method bidirectionally communicates with Unity via MQTT (MQTTCommunication class). Specifically, it sends
        motion and lick data to Unity and receives and executes water reward triggers from Unity. Additionally, it
        monitors for unexpected Unity shutdown messages and executes the necessary recovery steps if one is received.
        ...
        # If the animal has changed its position since the previous cycle, updates the position and sends the data to
        # Unity. Since Unity expects to be sent the delta (change) in animal position, rather than the current absolute
        ...
        if self._unity.has_data:
            ...
            if topic == self._unity_termination_topic:
                ...
                # Generates a new log entry with message ID code 5. This code is statically used to indicate that the
    ...
    def _ui_cycle(self) -> None:
        ...
        if self._ui.exit_signal:
            ...
            # If the user confirms runtime termination, braks the ui cycle to expedite the runtime shutdown sequence.
            if self._terminated:
                return
    ...
    def _resume_runtime(self) -> None:
        ...
        # state. Note, since the time is in microseconds, the time is first converted to seconds and then added to the
        # paused time tracker.
        pause_time = round(
        ...
        if self._unity_terminated:
            # When Unity game cycles, it resets the sequence of VR wall cues. This re-queries the new wall cue
            # sequence to enable accurate tracking of animal's position in VR after reset.
            self._get_cue_sequence()
        ...
        if self._mesoscope_terminated:
            # Although the Mesoscope termination flag typically indicates that the system is not able to restart the
            # acquisition using automated triggers, since this method falls back to requesting user assistance, it
            # should be safe here.
            self._start_mesoscope()
