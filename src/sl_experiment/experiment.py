...
@dataclass()
class _SessionDescriptor(YamlConfig):
    """This class is used to save the basic description information for each training and experiment session as a .yaml
    file.

    Primarily, the file generated by this class is used to provide experimenters with critical session information,
    such as the session type and the total volume of water dispensed during runtime. Additionally, this data may be used
    by some of our processing pipelines to generate the final dataset used for analysis and publication.
    """
    ...
    "experiment". This field is updated by the BehaviorTraining and the MesoscopeExperiment class as part of their 
    termination sequence.
    """
    dispensed_water_volume_ul: float = 0.0
    """Stores the total water volume, in microliters, dispensed during runtime. This field is updated by the 
    BehaviorTraining and the MesoscopeExperiment class as part of their termination sequence."""
...
class _BehavioralTraining:
    ...
    def stop(self) -> None:
        ...
        # Generates and dumps a SessionDescriptor.yaml file into the root raw_data session directory.
        descriptor = _SessionDescriptor(
            session_type=("lick_training" if self._lick_training else "run_training"),
            dispensed_water_volume_ul=self._microcontrollers.total_delivered_volume,
        )
        descriptor.to_yaml(file_path=self._session_data.session_descriptor_path)
        ...
        console.echo(message=message, level=LogLevel.SUCCESS)
        message = (
            f"During runtime, the system dispensed ~{descriptor.dispensed_water_volume_ul} uL of water to the animal. "
